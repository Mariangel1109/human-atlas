...
export function buildObjectives(c?:VliConsultation):VliObjective[]{
 ...
 if((n(c.alt)&&c.alt>30)||(n(c.ast)&&c.ast>30)||n(c.steatosis_grade)&&c.steatosis_grade>0) out.push({id:'liver',domain:'Hígado',finding:`ALT ${n(c.alt)?c.alt:'—'} / AST ${n(c.ast)?c.ast:'—'}${n(c.steatosis_grade)?` · esteatosis G${c.steatosis_grade}`:''}`,target:'Mejorar perfil hepático y reducir esteatosis; objetivo morfológico definido por seguimiento clínico/imaginológico.',action:'Cuantificar riesgo fibrótico (p. ej., FIB-4 cuando estén los datos), revisar causas secundarias y tratar factores cardiometabólicos.',review:'12–24 semanas según severidad',priority:(n(c.steatosis_grade)&&c.steatosis_grade>=2)?'Alta':'Media'});
 ...
 if(n(c.grip_strength)||n(c.vo2_estimate)||n(c.muscle_mass_pct)) out.push({id:'function',domain:'Función y músculo',finding:`Fuerza ${n(c.grip_strength)?c.grip_strength:'—'} kg · VO₂ ${n(c.vo2_estimate)?c.vo2_estimate:'—'} · músculo ${n(c.muscle_mass_pct)?c.muscle_mass_pct:'—'}%`,target:'Mejorar reserva funcional respecto al punto basal del paciente.',action:'Prescribir progresión de fuerza y capacidad aeróbica según tolerancia y riesgo.',review:'8–12 semanas',priority:'Seguimiento'});
